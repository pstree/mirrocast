...
class UpnpHttpServer(
    ...
    fun start() {
        if (serverJob?.isActive == true) {
            Log.w(TAG, "UPnP HTTP server already running")
            return
        }
        ...
        serverJob = scope.launch {
            try {
                ...
                Log.e(TAG, "UPnP HTTP server error", e)
            }
        }
    }
    ...
    fun stop() {
        try {
            ...
            Log.e(TAG, "Error stopping UPnP HTTP server", e)
        }
    }
    ...
    private fun handleClient(socket: Socket) {
        socket.use { client ->
            try {
                ...
                Log.e(TAG, "Error handling UPnP client", e)
            }
        }
    }
    ...
    private fun handleAvTransportControl(output: OutputStream, body: String, headers: Map<String, String>) {
        ...
        Log.i(TAG, "AVTransport action: $action")

        // 调试：打印完整请求体，帮助排查不同发送端的 SOAP 格式
        if (action == "SetAVTransportURI") {
            Log.d(TAG, "SetAVTransportURI body (first 1000 chars): ${body.take(1000)}")
        }
        ...
        val response = when (action) {
            "SetAVTransportURI" -> {
                ...
                if (uri.isBlank()) {
                    Log.e(TAG, "SetAVTransportURI: Empty URI! Trying fallback parsing...")
                    ...
                    if (fallbackUri.isNotBlank()) {
                        Log.i(TAG, "Found URI via fallback: $fallbackUri")
    ...
    private fun handleRenderingControl(output: OutputStream, body: String, headers: Map<String, String>) {
        val action = SoapParser.extractAction(headers["SOAPACTION"] ?: headers["SOAPAction"])
        Log.i(TAG, "RenderingControl action: $action")
    ...
    private fun handleConnectionManager(output: OutputStream, body: String, headers: Map<String, String>) {
        val action = SoapParser.extractAction(headers["SOAPACTION"] ?: headers["SOAPAction"])
        Log.i(TAG, "ConnectionManager action: $action")
